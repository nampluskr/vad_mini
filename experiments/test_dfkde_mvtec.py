# experiments/test_dfkde_mvtec.py
...
if __name__ == "__main__":
    ...
    from defectvad.models.dfkde.trainer import DfkdeTrainer

    if 1:
        trainer = DfkdeTrainer(backbone="resnet18", layers=["layer4"], feature_scaling_method="scale")
        trainer.fit(train_loader, max_epochs=1)
        results = trainer.validate(test_loader)
        print(">> " + ", ".join([f"{k}:{v:.3f}" for k, v in results.items()]))
    if 1:
        trainer = DfkdeTrainer(backbone="wide_resnet50_2", layers=["layer4"], feature_scaling_method="scale")
        trainer.fit(train_loader, max_epochs=1)
        results = trainer.validate(test_loader)
        print(">> " + ", ".join([f"{k}:{v:.3f}" for k, v in results.items()]))
